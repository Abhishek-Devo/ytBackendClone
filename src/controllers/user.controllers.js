...
const getUserChannelProfile=asyncHandler(async(req,res)=>{
  const {username}=req.params//to get something from url 

  if(!username?.trim()){
    throw new ApiError(400,"Username is required");
  }

  const channel = await User.aggregate( //aggregate is a method to use aggreagation pipeline
    [
      {
        $match:{
          username:username?.toLowerCase()
      ...
      {//structuring the information
        $addFields:{
          subscribersCount:{
            $size:"$subscribers" //$ sign if we named a variable by us or we created it.
          },
          channelsSubscribedToCount:{
              $size:"$subscribedTo"
          },
          //in case we are not logged in,show subscribe buttton
          isSubscribed:{ //value is going to be boolean
            $cond:{
              if:{
        ...
        //project/send only the necessary data
        $project:{
          fullname:1,
          username:1,
          avatar:1,
...
const getWatchHistory=asyncHandler(async(req,res)=>{
  const user=await User.aggregate([
    {
      $match:{ //in aggregation pipeline we give mongoose object id not string
...
export { registerUser, loginUser,logoutUser,refreshAccessToken,updateAccountDetails,updateUserCoverImage,changeCurrentPassword,updateUserAvatar,getCurrentUser,getUserChannelProfile,getWatchHistory};
